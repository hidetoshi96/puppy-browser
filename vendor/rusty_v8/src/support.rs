...
impl Hasher for TypeIdHasher {
  ...
  fn write_u64(&mut self, value: u64) {
    ...
    //
    // NOTE (puppy patch): the `std::any::TypeId` used to be 8 bytes long, and
    // this hasher asserted that here. Since Rust 1.72 `TypeId` is a 128-bit
    // value (see https://github.com/rust-lang/rust/pull/103449), so that
    // assertion no longer holds. The `Hash` implementation for `TypeId` still
    // feeds only its lower 64 bits to `Hasher::write_u64`, which is exactly
    // what this hasher is optimized for, hence hashing keeps working as
    // intended. The size/alignment assertions were therefore removed.
